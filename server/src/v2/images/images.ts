...
export const imagesRouter = router({
    ...
    /**
     * Mirrors an image and returns the URL of the flipped copy.
     *
     * Only the caller's own field is moved to the new URL. Anything else
     * referencing the old one still shows the original, so a shared image can
     * currently read as flipped on one screen and not another. Repointing the
     * rest waits on the facets index, which is what will locate references
     * without having to know in advance which fields hold them.
     */
    flip: procedure
